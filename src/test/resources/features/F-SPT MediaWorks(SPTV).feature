Feature: ClearPC_Features_SPT
  ...
  Scenario: Login_Logout_Feature
    ...
    @T10EssenceDelete_ClearPC
    Scenario: Validate_Essence_Delete_feature
    Given Open Browser and naviagate to the ClearPC portal
    When Enter the valid user name in username input field
    Then Click on Continue button of ClearPC
    And Enter the Valid password in the Password input field
    ...
    And Navigate to subfolder
    And Double Click on Asset and go to essence tab
    And Click on Delete Option for 2ndary Essence
    And Validate Essence Delete is initiated
    And  Click on Logout button from ClearPC
    Then Quit the browser for ClearPC
    
    #We have Open issue in portal for below scenario
    @T11EssenceEdit_ClearPC
    Scenario: Validate_Essence_Edit_feature
    Given Open Browser and naviagate to the ClearPC portal
    When Enter the valid user name in username input field
    Then Click on Continue button of ClearPC
    And Enter the Valid password in the Password input field
    ...
    And Navigate to subfolder
    And Double Click on Asset and go to essence tab
    And Click on Edit Option for 2ndary Essence
    And Enter Mandatory fields
    And Validate Essence Edit is initiated
    And  Click on Logout button from ClearPC
    Then Quit the browser for ClearPC
    
    @T12AssetShare_ClearPC
    Scenario: Validate_Asset_Share_feature
    Given Open Browser and naviagate to the ClearPC portal
    When Enter the valid user name in username input field
    Then Click on Continue button of ClearPC
    And Enter the Valid password in the Password input field
    And Click on Login button of ClearPC
    And Navigate to subfolder
    And Select Test file
    And Select Share option from dropdown
    And Enter valid email id and select share end date
    And Validate Asset Share is initiated successfully
    And Validate Asset Share entry is displaying in Inbox and  Outbox dashboard
    And  Click on Logout button from ClearPC
    Then Quit the browser for ClearPC
    
    @T13ScreenerPublish_ClearPC
    Scenario: Validate_Screener_Publish_feature
    Given Open Browser and naviagate to the ClearPC portal
    When Enter the valid user name in username input field
    Then Click on Continue button of ClearPC
    And Enter the Valid password in the Password input field
